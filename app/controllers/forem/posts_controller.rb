module Forem
  class PostsController < Forem::ApplicationController
    ...
    def show
      find_post
      # update notification
      update_read_notification(params[:ref_notify])
    ...
    def create_successful
      ...
      respond_to do |format|
        format.html { redirect_to forum_topic_url(@topic.forum, @topic, pagination_param => @topic.last_page) }
        format.json  { render json: @post.to_json(include: {user: { methods: [:avatar]}, replies: {except: []}}, methods: [:master_post, :reply_url]) }
      end
    end
    ...
    def create_failed
      ...
      respond_to do |format|
        ...
        format.json  { render json: @post.errors.full_messages.to_json }
      end
    end
